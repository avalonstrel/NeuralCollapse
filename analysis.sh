...
seeds="97 177"
metric_types="stn"  #margin, sep_loss, sep_stn, sep_stn_loss, stn, gstn, proj_dist, norm_dist
gstn_dim=1
gpu_num=2
max_num=9
curr_num=0
        ...
        do
            ...
            do
                if [ "${metric_types}" = "sep_loss" ] | [ "${metric_types}" = "sep_stn" ] | [ "${metric_types}" = "sep_stn_loss" ]; then
                    ...
                    tag="conv-"${metric_types}"-norm"${norm_type}"-"${dist_type}"-na-block-in-down"
                elif [ "${metric_types}" = "gstn" ]; then
                    tag="conv-"${metric_types}"-dim"${gstn_dim}"-nr-block-in-down-rand"
                else
                    tag="conv-"${metric_types}"-allcs-block-in-down"
                fi
                date=$seed
